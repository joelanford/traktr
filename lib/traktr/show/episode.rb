    ...
    class Episode < Endpoint
      def comments(title, season, episode)
        parse_response self.class.get(
          '/' + File.join('comments.json', @client.api_key, title, season.to_s,
                          episode.to_s))
      end

      def stats(title, season, episode)
        parse_response self.class.get(
          '/' + File.join('stats.json', @client.api_key, title, season.to_s,
                          episode.to_s))
      end

      def summary(title, season, episode)
        parse_response self.class.get(
          '/' + File.join('summary.json', @client.api_key, title, season.to_s,
                          episode.to_s))
      end
      ...
      def watchingnow(title, season, episode)
        parse_response self.class.get(
          '/' + File.join('watchingnow.json', @client.api_key, title,
                          season.to_s, episode.to_s))
      end

      def library(show, episodes)
        data = get_data(show, episodes)
      ...
      def get_data(show, episodes)
        Hash.new(
          username: @client.username, password: @client.password,
          title: show.title, year: show.year, imdb_id: show.imdb_id,
          tvdb_id: show.tvdb_id, episodes: episodes.map do |e|
